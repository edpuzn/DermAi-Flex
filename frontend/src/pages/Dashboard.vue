<template>
  <div class="min-h-screen bg-gray-50 text-gray-900 font-sans pb-24">
    ...
    <header class="bg-white border-b border-gray-200 sticky top-0 z-10 shadow-sm">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
        <div class="flex items-center space-x-3">
          ...
          <div>
            <div class="flex items-center gap-2">
              ...
              <span v-if="isDemoMode" class="bg-amber-100 text-amber-700 text-[10px] px-2 py-0.5 rounded-full font-black border border-amber-200 animate-pulse">{{ $t('login.demoAccess') }}</span>
            </div>
        ...
        <div class="flex items-center space-x-4">
          ...
          <router-link to="/history" class="p-2 text-gray-400 hover:text-indigo-600 transition-colors" :title="$t('nav.home')">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
          </router-link>
          <button @click="handleLogout" class="p-2 text-gray-400 hover:text-red-500 transition-colors" :title="$t('login.submit')">
    ...
    <!-- Loading Overlay -->
    <div v-if="store.isAnalyzing" class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/60 backdrop-blur-md">
      <div class="bg-white rounded-2xl p-8 max-w-sm w-full shadow-2xl text-center">
        <div class="relative w-20 h-20 mx-auto mb-6">
          <div class="absolute inset-0 border-4 border-indigo-100 rounded-full"></div>
          <div class="absolute inset-0 border-4 border-indigo-600 rounded-full border-t-transparent animate-spin"></div>
        </div>
        <h3 class="text-xl font-bold text-gray-900 mb-2">{{ $t('action.processing') }}</h3>
        <p class="text-sm text-gray-500 mb-6">{{ $t('dashboard.howItWorksDesc') }}</p>
        
        <div class="space-y-3">
          <div class="flex items-center gap-3 text-left p-3 rounded-lg border transition-colors" :class="store.currentStep === 'uploading' ? 'border-indigo-200 bg-indigo-50' : 'border-gray-100'">
            <div class="w-2 h-2 rounded-full" :class="store.currentStep === 'uploading' ? 'bg-indigo-600 animate-pulse' : 'bg-gray-300'"></div>
            <span class="text-xs font-bold uppercase tracking-wider" :class="store.currentStep === 'uploading' ? 'text-indigo-700' : 'text-gray-400'">1. {{ $t('action.ready') }}</span>
          </div>
          <div class="flex items-center gap-3 text-left p-3 rounded-lg border transition-colors" :class="store.currentStep === 'analyzing' ? 'border-indigo-200 bg-indigo-50' : 'border-gray-100'">
            <div class="w-2 h-2 rounded-full" :class="store.currentStep === 'analyzing' ? 'bg-indigo-600 animate-pulse' : 'bg-gray-300'"></div>
            <span class="text-xs font-bold uppercase tracking-wider" :class="store.currentStep === 'analyzing' ? 'text-indigo-700' : 'text-gray-400'">2. {{ $t('action.analyzingBtn') }}</span>
          </div>
          <div class="flex items-center gap-3 text-left p-3 rounded-lg border transition-colors" :class="store.currentStep === 'finishing' ? 'border-indigo-200 bg-indigo-50' : 'border-gray-100'">
            <div class="w-2 h-2 rounded-full" :class="store.currentStep === 'finishing' ? 'bg-indigo-600 animate-pulse' : 'bg-gray-300'"></div>
            <span class="text-xs font-bold uppercase tracking-wider" :class="store.currentStep === 'finishing' ? 'text-indigo-700' : 'text-gray-400'">3. {{ $t('results.ragContext') }}</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <UploadCard 
          ...
          @file-selected="(f) => store.setImageFile(f)"
          @error="showError"
          :file="store.imageFile"
          @remove="store.setImageFile(null)"
        >
          <template #icon>
        ...
        <UploadCard 
          ...
          @file-selected="(f) => store.setPdfFile(f)"
          @error="showError"
          :file="store.pdfFile"
          @remove="store.setPdfFile(null)"
        >
          <template #icon>
        ...
        <UploadCard 
          ...
          @remove="(index) => store.combinedFiles.splice(index, 1)"
          :highlight="true"
        >
           <template #icon>
...
onMounted(async () => {
  try {
    ...
    const healthBase = apiBase.replace('/api', '/health')
    const res = await axios.get(healthBase)
...
const canAnalyze = computed(() => {
  ...
  return store.imageFile !== null || 
         store.pdfFile !== null || 
         store.combinedFiles.length > 0
})
...
const handleCombinedSelected = (files) => {
  ...
  store.setCombinedFiles([...store.combinedFiles, ...newFiles].slice(0, 2))
}
